...
function materialNode(fullText) {
	...
	let materialName = headOfDataObject(fullText);
	...
	while (true) {
		...
		if (nextToken.nodeData == '') {
			...
			const textureName = textureFilenameNode(fullText.substring(node.valueLength));
...
function meshNormalNode(fullText, mesh) {
	...
	let head = headOfDataObject(fullText);
...
function meshMaterialListNode(fullText, mesh) {
	...
	while (true) {
		...
		if (nextToken.nodeData == '') {
			...
			// inlined material
			const material = materialNode(fullText.substring(node.valueLength));
...
function skinWeightsNode(fullText, mesh) {
	let node = new Types.ExportedNode(mesh);
	const head = headOfDataObject(fullText);
	node.updateExport(head);
	node.updateExport(StringUtils.readUntilNextNonWhitespace(fullText.substring(node.valueLength)));
...
function meshNode(fullText) {
	const meshName = headOfDataObject(fullText);
	...
	while (true) {
		...
		if (nextToken.nodeData == '') {
			...
			const meshNormals = meshNormalNode(fullText.substring(node.valueLength), mesh);
			node.updateExport(meshNormals);
			mesh = meshNormals.nodeData;
		} else if (nextToken.nodeData == 'MeshTextureCoords') {
			const meshTextureCoords = meshTextureCoordsNode(fullText.substring(node.valueLength), mesh);
			node.updateExport(meshTextureCoords);
			mesh = meshTextureCoords.nodeData;
		} else if (nextToken.nodeData == 'MeshVertexColors') {
			const meshVertexColors = meshVertexColorsNode(fullText.substring(node.valueLength), mesh);
			node.updateExport(meshVertexColors);
			mesh = meshVertexColors.nodeData;
		} else if (nextToken.nodeData == 'MeshMaterialList') {
			const meshMaterialList = meshMaterialListNode(fullText.substring(node.valueLength), mesh);
			...
			const skinWeights = skinWeightsNode(fullText.substring(node.valueLength), mesh);
...
function animationSetNode(fullText) {
	...
	while (true) {
		...
		if (nextToken.nodeData == '') {
			...
			const animNode = animationNode(fullText.substring(node.valueLength), animation);
...
function frameNode(fullText, parentFrame) {
	...
	while (true) {
		...
		if (nextToken.nodeData == '}') {
			...
			const frmNode = frameNode(fullText.substring(node.valueLength), frame);
			node.updateExport(frmNode);
			frame = frmNode.nodeData;
		} else if (nextToken.nodeData == 'FrameTransformMatrix') {
			const transformMatrix = transformationMatrixNode(fullText.substring(node.valueLength));
			node.updateExport(transformMatrix);
			frame.transformation = transformMatrix.nodeData;
		} else if (nextToken.nodeData == 'Mesh') {
			const mshNode = meshNode(fullText.substring(node.valueLength));
...
export {
	...
	materialNode,
	meshNormalNode,
	meshTextureCoordsNode,
	meshVertexColorsNode,
	meshMaterialListNode,
	unknownNode,
	skinWeightsNode,
	meshNode,
	animTicksPerSecondNode,
